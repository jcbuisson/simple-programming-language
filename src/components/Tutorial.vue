...
<script>
   ...
   export default {
      ...
      "the value of `input` is changed accordingly. The expression `input * input * input` is computed (it makes `1728`)",
      ...
      "When this first instruction is executed, control goes to next instruction `stop` at line 2, whose execution halts the program.",
      "",
      ...
      "",
      "The program is a simple sequence of drawing instructions, which all start by the prefix `pen_`.",
      "At line 0 there is a new kind of comment, enclosed by `/*` and `*/`, which may span several lines.  ",
      ...
      "At line 5 a new turn of `-90` orients it back to the right, then a move of `100` places the pen at coordinates `(100, 100)`.  ",
...
"       stop",
      ...
      "The `memory` array is the working memory of a program, each of its slots being accessible for reading or writing.",
      "It is potentially infinite, from index 0 upwards (negative indexes are not allowed).  ",
      "This index is generally called the `address` by programmers. For example in the picture above, they would say that the value `1` is located at address 0.",
      "You can store in the slots of the `memory` array any kind of integer or real number, but no other types of data.",
      "We'll see later how to represent more complex types of data, such as strings or sets, with such numbers.",
...
"         go here",
      ...
      "`memory[0] + 1` is an **expression**, that is a formula to be evaluated. The result of this evaluation is then copied to the destination, here `memory[0]`.  ",
...
"       (2 * 3.1415 * m[10] + m[11]) / (m[0] + m[1])",
      ...
      "For exemple, the string `'Hello world'` will be represented by the sequence `72, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100, 33, 0`,",
      ...
      "Now memory location at address `0` is interesting: its content is not a piece of data,",
      ...
      "In the following situation, it evaluates to `108`, since the current character address (in `memory[0]`) is `4`:",
      "",
